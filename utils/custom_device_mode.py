...
# (b) Checks for kwargs of the form `device="npu:i"`
# (c) Turns those into custom device objects: `device=npu_module.custom_device(i)`
# (d) Forwards the call along into pytorch.
class npuDeviceMode(TorchFunctionMode):
    def __torch_function__(self, func, types, args=(), kwargs=None):
        if kwargs is None:
            kwargs = {}
        if 'device' in kwargs and 'npu' in kwargs['device']:
            device_and_idx = kwargs['device'].split(':')
            if len(device_and_idx) == 1:
                # Case 1: No index specified
                kwargs['device'] = npu_module.custom_device()
            else:
                # Case 2: The user specified a device index.
                device_idx = int(device_and_idx[1])
                kwargs['device'] = npu_module.custom_device(device_idx)
